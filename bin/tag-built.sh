...
tag=$(cat build/unsplash.php | grep 'Version:' | sed 's/.*: //' | sed 's/-[0-9]\{8\}T[0-9]\{6\}Z-[a-f0-9]*$//')
if [[ -z "$tag" ]]; then
    echo "Error: Unable to determine tag from build/unsplash.php."
    exit 1
fi
...
echo "See https://github.com/xwp/unsplash-wp/releases/tag/$built_tag"
echo "For https://github.com/xwp/unsplash-wp/releases/tag/$tag"
