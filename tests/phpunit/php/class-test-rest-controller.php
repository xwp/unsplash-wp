...
class Test_Rest_Controller extends WP_Test_REST_Controller_Testcase {
	...
	public function test_get_items_via_ajax() {
		...
		foreach ( $data as $photo_object ) {
			$expected_keys = [
				...
				'editLink',
				'meta',
				'originalUnsplashImageURL',
				'originalUnsplashImageName',
				'sizes',
			];
			$this->assertEquals( $expected_keys, array_keys( $photo_object ) );
		}
	...
	public function test_prepare_item_for_response_for_ajax() {
		...
		$expected = [
			'id'                        => 'rO8TdlRrOo0',
			'title'                     => '',
			'filename'                  => 'waldemar-brandt-ro8tdlrroo0-unsplash.jpeg',
			...
			'unsplash_order'            => 0,
			'color'                     => '#F6F7FB',
			'unsplashAuthorLink'        => 'https://unsplash.com/@waldemarbrandt67w?utm_source=WordPress-XWP&utm_medium=referral&utm_campaign=api-credit',
			'originalUnsplashImageURL'  => 'https://unsplash.com/photos/rO8TdlRrOo0?utm_source=WordPress-XWP&utm_medium=referral&utm_campaign=api-credit',
			'originalUnsplashImageName' => 'Unsplash',
		];
