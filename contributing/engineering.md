...
```bash
git clone git@github.com:xwp/unsplash-wp.git unsplash
cd unsplash
```
...
This will create an `unsplash.zip` in the plugin directory which you can install. The contents of this ZIP are also located in the `build` directory which you can `rsync` somewhere as well if needed.

## Creating a pre-release

1. Create changelog draft on [Wiki page](https://github.com/xwp/unsplash-wp/wiki/Release-Changelog-Draft).
1. Check out the branch intended for release (`develop` for major, `x.y` for minor) and pull latest commits.
1. Bump plugin versions in `unsplash.php`.
1. Do `npm install`.
1. Do `npm run build` and install the `unsplash.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. [Draft new release](https://github.com/xwp/unsplash-wp/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor).
    ...
    1. Attach the `unsplash.zip` build to the release.
...
1. Check out the branch intended for release (`develop` for major, `x.y` for minor) and pull latest commits.
1. Do `npm install`.
1. Bump plugin versions in `unsplash.php`. Ensure patch version number is supplied for major releases, so `1.2-RC1` should bump to `1.2.0`.
1. Ensure "Tested Up To" is updated to current WordPress version.
1. Do `npm run build` and install the `unsplash.zip` onto a normal WordPress install running a stable release build; do smoke test to ensure it works.
1. Optionally do sanity check by comparing the `build` directory with the previously-deployed plugin on WordPress.org for example: `svn export https://plugins.svn.wordpress.org/unsplash/trunk /tmp/unsplash-trunk; diff /tmp/unsplash-trunk/ ./build/` (instead of straight `diff`, it's best to use a GUI like `idea diff`, `phpstorm diff`, or `opendiff`).
1. [Draft new release](https://github.com/xwp/unsplash-wp/releases/new) on GitHub targeting the required branch (`develop` for major, `x.y` for minor):
